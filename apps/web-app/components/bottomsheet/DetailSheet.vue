...
<template>
  <CoreSheet :is-open="detailSheet.isOpen" :snap-points="[280, 0]" @close="handleClose">
    <template #header>
      <div
        v-if="emergencyData && emergencyType"
      ...
      >
        <div class="flex gap-2 items-center min-w-0">
          <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 shrink-0">
            <Icon :icon="emergencyType.icon" class="text-red-500 text-xl" />
          </div>
          <div class="min-w-0">
            <h1 class="text-md leading-none m-0 text-neutral-800 font-semibold truncate">
              {{ emergencyData.name }}
            </h1>
            <p v-if="tripData" class="m-0 mt-1 leading-none text-[14px] text-neutral-500">
              {{ Math.floor(tripData.duration) * 2 }} menit dari lokasimu saat ini
            </p>
          </div>
        </div>
        <button
