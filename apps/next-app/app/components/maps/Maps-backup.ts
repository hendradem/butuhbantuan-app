...
import { emergencyService } from '@/store/api/services/emergency.service'
...
const MapsV2: React.FC<MapsProps> = () => {
    ...
    const [currentRegionID, setCurrentRegencyID] = useState('')
    ...
    // ===== api ======
    const { getEmergencybyRegion, refetchEmergencybyRegion } =
        useEmergencyApi(currentRegionID)
    ...
    const handleMapLoad = (): void => {
        getCurrentLocation((location: any) => {
            ...
            if (userLatitude && userLongitude) {
                ...
                // getAddressInfo(userLongitude, userLatitude).then((res) => {
                //     const regency = res[0]?.context[2]?.text

                //     if (regency) {
                //         getAvailableRegion(regency, userLongitude, userLatitude)
                //     }
                // })
    ...
    const handleMapClick = (e: any) => {
        ...
        console.log(getEmergencybyRegion)
        ...
        getAddressInfo(lng, lat).then((res) => {
            ...
            if (address && regency) {
                getAvailableRegion(regency, lng, lat)
    ...
    const drawCurrentMarkerLocation = (longitude: number, latitude: number) => {
        ...
        // getAddressInfo(longitude, latitude).then((res) => {
        //     const address = res[0]?.place_name
        //     const regency = res[0]?.context[2]?.text

        //     if (address && regency) {
        //         // getAvailableRegion(regency, longitude, latitude)
        //         updateFullAddress(address)
        //     }
        // })
    ...
    const getMatrixOfLocations = (
        longitude: number,
        latitude: number,
        isServiceAvailable: boolean
    ) => {
        console.log('matrix called')
        getDistanceMatrix(
            [longitude ?? 0, latitude ?? 0],
            getEmergencybyRegion?.data,
            isServiceAvailable
        ).then((res) => {
            console.log(res)
    ...
    const getAvailableRegion = async (
        ...
        await availableCityService.getByName(regencyName).then((res) => {
            ...
            if (res.data.length > 0) {
                ...
                const regencyID = result.regency_id
                ...
                toast.success(`${regencyName} tersedia`, { duration: 900 })
                setCurrentRegencyID(regencyID)
                getMatrixOfLocations(lng, lat, true)
                ...
                console.log('gaada')
                getMatrixOfLocations(lng, lat, false)
            }
        })
    }
