  ...
  return (
    <form>
      <div className="px-1.5">
        <div className={`gap-1 text-gray-800 flex items-center w-full p-0`}>
          ...
          {!isFullScreen && (
            ...
            >
              <Icon name="iconamoon:location-bold" className="text-xl" />
            </button>
          )}
        </div>
        ...
        {isFullScreen && locationResult?.length == 0 && !isLoading && (
          <div>
            ...
            <h3 className="text-center mx-3 text-[14px] text-neutral-500 font-normal">
              Cari lokasimu dengan nama jalan, nama desa, atau nama tempat
              terdekatmu
            </h3>
          </div>
        )}
        ...
        {!isLoading && locationResult.length !== 0 && isSearchBoxActive && (
          <div className="w-full mt-4 bg-white text-neutral-800">
            {locationResult?.map((item: LocationStateType, index) => {
              return (
                <div
                  key={index}
