...
definePageMeta({ title: "Live Map Ops", keepalive: true });
...
const { data: emergencyData, pending: unitsPending, refresh: refreshUnits } = await useAsyncData(
  "ops-live-units",
  () => get<{ data: any[] }>("/api/v1/emergency/").then((r) => r.data ?? []),
  { server: false },
);
  ...
  <div>
    <div class="page-subheader">
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 class="page-subheader-title">Live Map Ops</h1>
          <p class="page-subheader-desc">
            Kejadian aktif + lokasi unit · ETA estimasi dari HQ / tracking
          </p>
        </div>
        <button
          type="button"
